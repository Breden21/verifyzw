...
// First 2 digits: district code (e.g. 63 = Harare, 07 = Bulawayo, 30 = Gweru)
// Next 7 digits: sequence number
// Letter: checksum letter (A-Z)
// Last 2 digits: sub-district
...
// Strip spaces/dashes so enumerators can type 631234567A75 or 63 1234567 A 75
...
function isDuplicate(rawInput) {
  const norm = normaliseID(rawInput);
  return registrations.some(r => normaliseID(r.idNumber) === norm);
}

function registerMember(record) {
  const entry = { ...record, regId: `REG-${String(nextId++).padStart(4, "0")}`, timestamp: new Date().toISOString() };
...
function getStats() {
  ...
  const byWard = registrations.reduce((acc, r) => {
    acc[r.ward] = (acc[r.ward] || 0) + 1;
    return acc;
  }, {});
  return { total, verified, unverified, byWard };
}
...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .login-wrap {
    ...
    padding: 24px;
    background:
      radial-gradient(ellipse 80% 60% at 50% -20%, rgba(99,132,255,0.15) 0%, transparent 70%),
      radial-gradient(ellipse 60% 40% at 80% 120%, rgba(245,200,66,0.08) 0%, transparent 60%),
      var(--bg);
  }
  ...
  .login-card {
    ...
    border-radius: 20px;
    padding: 48px 40px;
    box-shadow: 0 32px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(99,132,255,0.05) inset;
  ...
  .login-logo-name { font-size: 18px; font-weight: 700; color: var(--text); letter-spacing: -0.3px; }
  ...
  .form-input:focus {
    ...
    box-shadow: 0 0 0 3px rgba(99,132,255,0.1);
  }

  .form-input.mono { font-family: 'JetBrains Mono', monospace; letter-spacing: 1px; }
  ...
  .btn-green:hover { transform: translateY(-1px); box-shadow: 0 8px 24px rgba(34,197,94,0.4); }
  ...
  .main {
    ...
    padding: 32px;
    background:
      radial-gradient(ellipse 60% 40% at 100% 0%, rgba(99,132,255,0.06) 0%, transparent 60%),
      var(--bg);
  }
  ...
  .enum-wrap {
    min-height: 100vh;
    background:
      radial-gradient(ellipse 80% 50% at 50% 0%, rgba(245,200,66,0.08) 0%, transparent 60%),
      var(--bg);
    padding: 24px 16px;
    display: flex;
    flex-direction: column;
...
// ─── ADMIN PORTAL ─────────────────────────────────────────────────────────────
function AdminPortal({ user, onLogout, refreshKey }) {
  ...
  const wardEntries = Object.entries(stats.byWard).sort((a, b) => b[1] - a[1]);
  ...
  return (
    <div className="app">
      ...
      <main className="main">
        {activeNav === "dashboard" && (
          <>
            <div className="page-header">
              ...
              <div className="page-sub">Live member registration overview</div>
            </div>
            ...
            <div className="stats-grid">
              ...
              <div className="stat-card green">
                ...
                <div className="stat-value green">{stats.verified}</div>
                <div className="stat-sub">{rate}% pass rate</div>
              </div>
              <div className="stat-card red">
                <div className="stat-label">Unverified</div>
                <div className="stat-value red">{stats.unverified}</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
              <div className="panel">
                <div className="panel-header">
                  <div className="panel-title"><Icon.MapPin /> Registrations by Ward</div>
                </div>
                <div className="panel-body">
                  {wardEntries.length === 0 ? (
                    <div className="empty-state"><Icon.MapPin /><p>No data yet</p></div>
                  ) : (
              ...
              <div className="panel">
                <div className="panel-header">
                  <div className="panel-title"><Icon.AlertTriangle /> Recent Unverified</div>
                </div>
                <div className="panel-body">
                  {registrations.filter(r => r.status === "UNVERIFIED").length === 0 ? (
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                      {registrations.filter(r => r.status === "UNVERIFIED").slice(-5).reverse().map(r => (
        ...
        {activeNav === "enumerators" && (() => {
          // Build per-enumerator stats
          ...
          const enumStats = enumNames.map(name => {
            ...
            const unverified = recs.filter(r => r.status === "UNVERIFIED").length;
            const rate = recs.length > 0 ? Math.round((verified / recs.length) * 100) : 0;
            return { name, total: recs.length, verified, unverified, rate, records: recs };
          ...
          return (
            <>
              <div className="page-header" style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                <div>
                  <div className="page-title">By Enumerator</div>
                  <div className="page-sub">Performance breakdown per field enumerator</div>
                </div>
                {selectedEnum && (
                  <button className="btn btn-ghost btn-sm" onClick={() => setSelectedEnum(null)}>← All Enumerators</button>
                )}
                </div>
                
                              {!selectedEnum ? (
                                enumStats.length === 0 ? (
                                  <div className="panel"><div className="empty-state" style={{ padding: "48px" }}><Icon.User /><p>No registrations yet. Enumerator data will appear here.</p></div></div>
                                ) : (
                                    ...
                                    {enumStats.map(e => (
                                      <div key={e.name} className="panel" style={{ cursor: "pointer" }} onClick={() => setSelectedEnum(e.name)}>
                                        <div style={{ padding: "20px 24px", display: "flex", alignItems: "center", gap: "20px" }}>
                                          <div className="user-avatar" style={{ width: "42px", height: "42px", fontSize: "14px", flexShrink: 0 }}>
                                          ...
                                          <div style={{ flexShrink: 0 }}>
                                            <div style={{ width: "80px", height: "6px", background: "var(--surface2)", borderRadius: "3px", overflow: "hidden" }}>
                                              <div style={{ width: `${e.rate}%`, height: "100%", background: e.rate >= 80 ? "var(--green)" : e.rate >= 50 ? "var(--gold)" : "var(--red)", borderRadius: "3px", transition: "width 0.6s ease" }} />
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                ...
                                <>
                                  <div className="panel" style={{ marginBottom: "20px" }}>
                                    <div style={{ padding: "20px 24px", display: "flex", gap: "24px", borderBottom: "1px solid var(--border)" }}>
                                      {[
                                        { label: "Total", value: selectedRecs.length, color: "var(--gold)" },
                                        { label: "Verified", value: selectedRecs.filter(r => r.status === "VERIFIED").length, color: "var(--green)" },
                                        { label: "Flagged", value: selectedRecs.filter(r => r.status === "UNVERIFIED").length, color: "var(--red)" },
                                        { label: "Pass Rate", value: selectedRecs.length > 0 ? `${Math.round((selectedRecs.filter(r => r.status === "VERIFIED").length / selectedRecs.length) * 100)}%` : "—", color: "var(--blue)" },
                                      ].map(s => (
                                        <div key={s.label}>
                                          <div style={{ fontSize: "11px", color: "var(--text-faint)", textTransform: "uppercase", letterSpacing: "1px", marginBottom: "4px" }}>{s.label}</div>
                          ...
                          <>
                            <div className="page-header" style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                              <div>
                                ...
                                <div className="page-sub">{registrations.length} total records</div>
                              </div>
                              <div className="search-bar">
                                <Icon.Search />
                                <input className="search-input" placeholder="Search name, ID, ward…" value={search} onChange={e => setSearch(e.target.value)} />
                              </div>
                            </div>
                            ...
                            <div className="panel">
                              {filtered.length === 0 ? (
                                <div className="empty-state" style={{ padding: "48px" }}>
                                  <Icon.Clipboard />
                                  <p>{search ? "No matching records" : "No registrations yet. Enumerators will appear here in real time."}</p>
                                </div>
                              ) : (
                                <div className="table-wrap">
                                  <table>
                                    ...
                                    <tbody>
                                      {[...filtered].reverse().map(r => (
                                        <tr key={r.regId}>
                                          ...
                                          <td>
                                            {r.status === "VERIFIED"
                                              ? <span className="badge badge-green"><Icon.Check />Verified</span>
                                              : <span className="badge badge-red"><Icon.X />Unverified</span>}
                                          </td>
                                          <td style={{ color: "var(--text-faint)", fontSize: "12px" }}>{new Date(r.timestamp).toLocaleTimeString()}</td>
                ...
                function EnumeratorPortal({ user, onLogout }) {
                  ...
                  const [lookupResult, setLookupResult] = useState(null); // null | { found: bool, voter: obj|null }
                  const [loading, setLoading] = useState(false);
                  const [step, setStep] = useState("input"); // input | form | success
                  ...
                  const handleSubmit = () => {
                    ...
                    const saved = registerMember(record);
                    setSubmitted(saved);
                    setStep("success");
                  };
                  ...
                  const handleReset = () => {
                    ...
                    setLookupResult(null);
                    setStep("input");
                    setForm({ phone: "", ward: "", notes: "" });
                    setSubmitted(null);
                    setLoading(false);
                  };
                
                  const sessionCount = registrations.filter(r => r.enumerator === user.name).length;
                            ...
                            <div className="id-input-panel">
                              ...
                              <div className="id-format-hint">
                                {idInput.trim() && normaliseID(idInput).length > 0
                                  ? <>Formatted: <span style={{color:"var(--gold)"}}>{formatID(normaliseID(idInput.trim())) || idInput.trim().toUpperCase()}</span></>
                              ...
                              <button className="verify-btn" onClick={handleVerify} disabled={!idInput.trim() || loading}>
                                {loading ? <span className="loading">Verifying…</span> : <><Icon.Search /> Verify ID</>}
                              </button>
                            </div>
                
                            <div style={{ textAlign: "center", color: "var(--text-faint)", fontSize: "12px", padding: "12px" }}>
                              Enter the member's national ID number to check against the voters' roll
                            </div>
                          </>
                        )}
                
                        {/* STEP: FORM (after lookup) */}
                          ...
                          <>
                            ...
                            {lookupResult.duplicate && (
                              <div className="result-card unverified" style={{ borderColor: "rgba(245,200,66,0.4)", background: "var(--gold-dim)" }}>
                                <div className="result-icon" style={{ background: "rgba(245,200,66,0.2)", color: "var(--gold)" }}>
                                  <Icon.AlertTriangle />
                                </div>
                                <div className="result-status" style={{ color: "var(--gold)" }}>⚠ Duplicate — Already Registered</div>
                                <div className="result-name" style={{ color: "var(--gold)", fontSize: "16px" }}>
                                  This ID number has already been submitted. Registration blocked.
                                </div>
                                <div style={{ marginTop: "12px" }}>
                                  <span className="mono-td" style={{ fontSize: "13px" }}>{formatID(normaliseID(idInput.trim()))}</span>
                                </div>
                              </div>
                            )}
                
                            {/* Result card */}
                            {!lookupResult.duplicate && <div className={`result-card ${lookupResult.found ? "verified" : "unverified"}`}>
                                ...
                                <>
                                  ...
                                  <div className="result-fields">
                                    <div className="result-field">
                                      <div className="result-field-label">National ID</div>
                                      <div className="result-field-value mono">{lookupResult.voter.id}</div>
                                    </div>
                                    <div className="result-field">
                                      <div className="result-field-label">Date of Birth</div>
                                      <div className="result-field-value">{lookupResult.voter.dob}</div>
                                    </div>
                                    <div className="result-field">
                                      <div className="result-field-label">Gender</div>
                                      <div className="result-field-value">{lookupResult.voter.gender === "M" ? "Male" : "Female"}</div>
                                    </div>
                                    <div className="result-field">
                                      <div className="result-field-label">Ward</div>
                                      <div className="result-field-value">{lookupResult.voter.ward}</div>
                                    </div>
                                  </div>
                                </>
                              ) : (
                            ...
                            {/* Additional fields — only show if not a duplicate */}
                            {!lookupResult.duplicate && <div className="member-form">
                              ...
                              <div className="form-group">
                                <label className="form-label">Notes {lookupResult.found ? "(optional)" : "(required — reason for unverified)"}</label>
                ...
                export default function App() {
                  ...
                  if (user.role === "admin") return <AdminPortal user={user} onLogout={() => setUser(null)} refreshKey={refreshKey} />;
